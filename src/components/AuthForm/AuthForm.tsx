...
import { signInEmail } from "../../../backend/signIn.tsx";
import { signInWithEmailAndPassword } from "firebase/auth";
import { auth, db } from "../../../backend/firebaseAuth.ts";
import { doc, getDoc } from "firebase/firestore";
import { setProfileName } from "../../pages/Profilepage/SetName.ts"
...
const AuthForm = () => {
  ...
  const checkIfUserExist = async () => {
    const isUser = await signInEmail(inputs.email, inputs.password);
    if (isUser) {
      ...
      if (userDoc.exists()) {
        ...
        console.log("Name from Firestore:", username);
  ...
  const handleAuth = async () => {
    ...
    const isUser = await checkIfUserExist();
    isLogin && isUser ? navigate("/") : navigate("/onboarding");
  };
  ...
  return (
    <Box border={"1px solid gray"} borderRadius={4} padding={5} shadow={"md"}>
      <VStack>
        ...
        <Text fontFamily="Bangers, cursive" fontWeight="bold" fontSize="4xl">
          {" "}
